...
class EditService:
    ...
    async def record(self, chat_id, message_id, user_id, username, original, new, original_time, edit_time):
        """Record an edit atomically so concurrent edit events cannot reuse a count."""
        assert self.db.conn
        async with self.db.conn.execute(
            """INSERT INTO edit_history
            (chat_id,message_id,user_id,username,original_text,new_text,original_time,edit_time,edit_count)
            SELECT ?,?,?,?,?,?,?,?, COALESCE(MAX(edit_count),0)+1
            FROM edit_history WHERE chat_id=? AND message_id=?
            RETURNING edit_count""",
            (chat_id, message_id, user_id, username, original, new, original_time, edit_time,
             chat_id, message_id),
        ) as cursor:
